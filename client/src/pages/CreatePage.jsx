...
const handleSubmit = async (e) => {
    ...
    // console.log(name, code, img, category, qty, price);

    try {
        const newData = {name, code, img, category, qty, price};
        const newFood = await axios.post('/api/v1/create-food', newData);
        return newFood;
    }catch (err) {
        console.error(err);
    }
                    ...
                    <form onSubmit={handleSubmit} className=''>
                        <div className='flex gap-2 my-5'>
                            <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor="title">
                                Food name
                                <input required={true} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' type="text" name="name" id="name"/>
                            </label>
                        ...
                        <input type="submit" className='cursor-pointer bg-purple-700 py-2 px-3 rounded text-white' value="Submit"/>
                    </form>
                </div>
            </div>
        </MasterLayout>
